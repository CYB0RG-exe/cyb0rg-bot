...
class Window(Frame):
    def __init__(self, master=None, saniye=0):
        ...
        try:
            ...
            self.esya_topla.grid(row=10, column=0, sticky=W)
            self.esya_tusu = Entry(F3, width=5)
            self.esya_tusu.insert(0, self.ayarlar['esya_topla'])
            self.esya_tusu.grid(row=10, column=1, sticky=W)
            self.esyaKilit()
            ...
            self.config_save_btn.grid(row=17, column=0, columnspan=4, pady=10)
            self.iksirKilit()
            keyboard.add_hotkey(self.ayarlar['stop'], self.stop_script)
        except Exception as e:
            messagebox.showerror('Value Error', f'config.json Hatalı Değerler içeriyor.\n{e}')
            os._exit(1)
        ...
        try:
            ...
            while True:
                ...
                if not self.skill_yak_var.get() or not int(time()) % (float(self.skill_suresi.get()) * 60) == 0 or self.skill1.get():
                    ...
                    sleep(self.ayarlar['at_inme_suresi'])
